...
def iniciar_sesion(root, entry_user, entry_contra, label_estado, btn_comenzar):
    ...
    if sesion_actual["jugador1"] is None: #El campo inicia vacío
        ...
        label_estado.config(text=f"✔ {jugador.nombre_usuario} ha sido seleccionado. Esperando jugador 2...", fg="green")
    ...
    #Si ya hay un jugador 1 registrado, se pasa al jugador 2
        ...
        label_estado.config(text=f"✔ {jugador.nombre_usuario} ha sido seleccionado. ¡Es hora de para comenzar!", fg="blue")
...
def mostrar_login(root):
    ...
    tk.Label(root, text="Defensa y Asalto de Base", font=FUENTE_TITULO, bg=COLOR_FONDO, fg=COLOR_TITULO).pack(pady=30)
...
def mostrar_mapa_y_defensor(root, jugador1, jugador2, faccion_defensor, faccion_atacante, estado=None):
    ...
    tk.Label(panel_izq, text=jugador1.nombre_usuario, font=("Arial", 11, "bold"), bg=COLOR_PANEL, fg=COLOR_TITULO).pack(pady=(15,0))
    tk.Label(panel_izq, text="(Defensor)", font=("Arial", 9, "italic"), bg=COLOR_PANEL, fg="gray").pack()

    label_dinero = tk.Label(panel_izq, text=f"💰 ${estado.dinero_defensor}", font=FUENTE_BOTON, bg=COLOR_PANEL, fg=COLOR_TITULO)
    label_dinero.pack(pady=10)

    tk.Label(panel_izq, text="Ronda", font=FUENTE_BOTON, bg=COLOR_PANEL, fg=COLOR_TITULO).pack(pady=(15,2))
    
    label_ronda = tk.Label(panel_izq, text=f"{estado.ronda_actual}", font=("Arial", 14, "bold"), bg=COLOR_PANEL, fg=COLOR_TITULO)
    label_ronda.pack()

    tk.Label(panel_izq, text="Torres", font=FUENTE_BOTON, bg=COLOR_PANEL, fg=COLOR_TITULO).pack(pady=(10,2))
    ...
    tk.Label(panel_izq, text="Muros", font=FUENTE_BOTON, bg=COLOR_PANEL, fg=COLOR_TITULO).pack(pady=(10,2))
    ...
    tk.Button(panel_izq, text="✔ Listo", font=FUENTE_BOTON, bg="green", fg="white", width=15, command=ir_a_fase_atacante).pack(pady=20)
...
def mostrar_seleccion_faccion(root, jugador1, jugador2):
    ...
    tk.Label(frame_j1, text="(Defensor)", font=("Arial", 9, "italic"),bg=COLOR_PANEL, fg="gray").pack(pady=2)
    label_seleccion_j1 = tk.Label(frame_j1, text="Sin selección",font=FUENTE_NORMAL, bg=COLOR_PANEL, fg="gray")
    ...
    tk.Label(frame_j2, text="(Atacante)", font=("Arial", 9, "italic"), bg=COLOR_PANEL, fg="gray").pack(pady=2)
    ...
    for clave, datos in FACCIONES.items():
        def hacer_elegir_j1(c=clave): #asegura que el boton escoja el valor correcto
            elegir_j1(c)
        tk.Button(frame_j1, text=datos["nombre"], font=FUENTE_NORMAL, bg=datos["color_torre"], fg="white", width=15, command=hacer_elegir_j1).pack(pady=3)
    ...
    for clave, datos in FACCIONES.items():
        def hacer_elegir_j2(c=clave): #asegura que el boton escoja el valor correcto
            elegir_j2(c)
        tk.Button(frame_j2, text=datos["nombre"], font=FUENTE_NORMAL, bg=datos["color_torre"], fg="white", width=15, command=hacer_elegir_j2).pack(pady=3)
...
def mostrar_fase_atacante(root, jugador1, jugador2, faccion_defensor, faccion_atacante, estado):
    ...
    #Elementos de interfaz
    tk.Label(panel_izq, text=jugador2.nombre_usuario, font=("Arial", 11, "bold"), bg=COLOR_PANEL, fg=COLOR_TITULO).pack(pady=(15,0))
    tk.Label(panel_izq, text="(Atacante)", font=("Arial", 9, "italic"),bg=COLOR_PANEL, fg="gray").pack()

    label_dinero = tk.Label(panel_izq, text=f"💰 ${estado.dinero_atacante}", font=FUENTE_BOTON, bg=COLOR_PANEL, fg=COLOR_TITULO)
    label_dinero.pack(pady=10)

    tk.Label(panel_izq, text="Ronda", font=FUENTE_BOTON, bg=COLOR_PANEL, fg=COLOR_TITULO).pack(pady=(5,2))
    tk.Label(panel_izq, text=f"{estado.ronda_actual}", font=("Arial", 14, "bold"), bg=COLOR_PANEL, fg=COLOR_TITULO).pack()
...
def mostrar_resultado_ronda(root, jugador1, jugador2, faccion_defensor, faccion_atacante, estado, ganador):
    ...
    tk.Label(root, text=f"🏆 {nombre_ganador} ganó la ronda", font=("Arial", 14, "bold"), bg=color_ganador, fg="white").pack(pady=10, ipadx=15, ipady=8)

    frame_marcador = tk.Frame(root, bg=COLOR_PANEL, padx=20, pady=15)
    frame_marcador.pack(pady=15)

    tk.Label(frame_marcador, text="Marcador", font=FUENTE_BOTON, bg=COLOR_PANEL, fg=COLOR_TITULO).pack(pady=(0,8))
    tk.Label(frame_marcador, text=f"{jugador1.nombre_usuario} (Defensor): {estado.rondas_defensor} ronda(s)", font=FUENTE_NORMAL, bg=COLOR_PANEL).pack()
    tk.Label(frame_marcador, text=f"{jugador2.nombre_usuario} (Atacante): {estado.rondas_atacante} ronda(s)", font=FUENTE_NORMAL, bg=COLOR_PANEL).pack()

    #Caso 1. Ganó defensor
    if estado.rondas_defensor >= 3: 
        tk.Button(root, text="Ver ganador", font=FUENTE_BOTON, bg=COLOR_BOTON, fg="white", width=20, command=lambda: mostrar_ganador_partida(root, jugador1, jugador2, "defensor")).pack(pady=20)
    ...
    #Caso 3: Ninguno ha ganado
        ...
        tk.Label(root, text=f"Próxima: Ronda {estado.ronda_actual + 1}", font=("Arial", 10, "italic"), bg=COLOR_FONDO, fg="gray").pack(pady=5)
...
def mostrar_ganador_partida(root, jugador1, jugador2, ganador):
    ...
    jugador_ganador.sumar_victoria("atacante")

    gestor.guardar() #Se guarda en Json

    #Elementos de interfaz
    tk.Label(root, text="¡Partida terminada!", font=FUENTE_TITULO, bg=COLOR_FONDO, fg=COLOR_TITULO).pack(pady=30)

    frame = tk.Frame(root, bg=color, padx=30, pady=20)
    frame.pack(pady=10)

    tk.Label(frame, text=f"🏆 {jugador_ganador.nombre_usuario}", font=("Arial", 18, "bold"), bg=color, fg="white").pack()
    tk.Label(frame, text=f"Rol: {rol_ganador.capitalize()}", font=("Arial", 11, "italic"), bg=color, fg="white").pack(pady=5)
...
def mostrar_top_jugadores(root):
    ...
    if not top_defensores: #Si no hay jugadores
        tk.Label(frame_def, text="Sin jugadores registrados", font=FUENTE_NORMAL, bg=COLOR_PANEL, fg="gray").pack()
    ...
    if not top_atacantes: #Si no hay atacantes
        tk.Label(frame_atk, text="Sin jugadores registrados",font=FUENTE_NORMAL, bg=COLOR_PANEL, fg="gray").pack()
